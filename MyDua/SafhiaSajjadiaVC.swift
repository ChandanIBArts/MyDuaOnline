...
class SafhiaSajjadiaVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        hijriApiChangeDate()
        musicIdx = 0
        safhiaTableView.dataSource = self
        safhiaTableView.delegate = self
        searchBar.delegate = self
        fetchFavDuaList()
        startTimer()
        globalControlerName = "SafhiaSajjadiaVC"
...
//    @IBAction func updateSound(_ sender: UISlider) {
    ...
    @objc func makeFav(_ sender:UITapGestureRecognizer){
        ...
        if strLang == "English"{
           ...
           // print(arr.name)
            ...
            objSingleton.makeFavourite(userId: userId, audioId: duaList[tag].id as? Int ?? 0,typeOfAudio: "sahifafavaudio"){
                str in
                if str == "success"{
            ...
            let tag = sender.view?.tag as? Int ?? 0
            objSingleton.makeFavourite(userId: userId, audioId: Hindi_duaList[tag].id as? Int ?? 0,typeOfAudio: "sahifafavaudio"){
                str in
                if str == "success"{
                    self.fetchFavDuaList()
                }else{
            ...
            let tag = sender.view?.tag as? Int ?? 0
            objSingleton.makeFavourite(userId: userId, audioId: Arabic_duaList[tag].id as? Int ?? 0,typeOfAudio: "sahifafavaudio"){
                str in
                if str == "success"{
                    self.fetchFavDuaList()
                }else{
            ...
            let tag = sender.view?.tag as? Int ?? 0
            objSingleton.makeFavourite(userId: userId, audioId:Gujrati_duaList[tag].id as? Int ?? 0,typeOfAudio: "sahifafavaudio"){
                str in
                if str == "success"{
                    self.fetchFavDuaList()
                }else{
...
//                    musicIdx = tag
                    ...
                    let cell = safhiaTableView.cellForRow(at: idx) as! SahifaSajjadiaTableViewCell
...
//                self.loadingView.isHidden = true
    ...
    func fetchMusicUrl(with idx:Int,and cell:SahifaSajjadiaTableViewCell) {
        var url = ""
        let objSingleton = SingletonApi()
...
//                        self.dailysafhiaTableView.reloadData()
                    }
    ...
    func fetchDuaList() {
        objSingleton.sahfia_sajjadia_API(onSuccess: { response in
            ...
            DispatchQueue.main.async {
                ...
                if self.musicIdx<self.duaList.count{
                    self.ButtonPlay.setImage(UIImage(named: "audio_play"), for: UIControl.State.normal)
                    self.setupMusicUI(url: self.duaList[self.musicIdx].file as? String ?? "")
                }
...
extension SafhiaSajjadiaVC: UIPickerViewDelegate, UIPickerViewDataSource {
